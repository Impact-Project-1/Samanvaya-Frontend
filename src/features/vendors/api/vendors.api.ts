...
import { VendorSchema, type Vendor, type VendorFilterParams } from "../schemas";
import { z } from "zod";
...
export async function updateVendor(id: string, vendor: Partial<Vendor>): Promise<Vendor> {
  return apiClient.put<Vendor>(endpoints.vendors.byId(id), vendor, VendorSchema);
}
...
export async function filterVendors(params: VendorFilterParams): Promise<Vendor[]> {
  ...
  return apiClient.get<Vendor[]>(endpoints.vendors.filter, z.array(VendorSchema), queryParams);
}
